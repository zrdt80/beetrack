...
def create_hive(session, apiary, name="Hive I", last_date=None):
    h = models.Hive(name=name, apiary_id=apiary.id, status="active", last_inspection_date=last_date)
    session.add(h)
    session.flush()
    return h
...
def create_inspection_entity(session, hive, date):
    ins = models.Inspection(hive_id=hive.id, date=date, notes="", temperature=30.0, disease_detected="none")
    session.add(ins)
    session.flush()
    return ins
...
def test_list_inspections_and_for_hive(db_session, admin_user):
    ...
    create_inspection_entity(db_session, hive1, datetime.now(timezone.utc) - timedelta(days=2))
    create_inspection_entity(db_session, hive2, datetime.now(timezone.utc) - timedelta(days=1))
...
def test_update_inspection_reassigns_hive_and_recalc(db_session, admin_user):
    ...
    assert abs((_aware(h1.last_inspection_date) - _aware(older)).total_seconds()) < 3
