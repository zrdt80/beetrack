...
def _seed_logs(session):
    rows = [
        models.Log(timestamp=datetime.now(timezone.utc), event="Started system", level="info"),
        models.Log(timestamp=datetime.now(timezone.utc), event="Operation success", level="success"),
        models.Log(timestamp=datetime.now(timezone.utc), event="Minor warning", level="warning"),
        models.Log(timestamp=datetime.now(timezone.utc), event="Critical error occurred", level="error"),
        models.Log(timestamp=datetime.now(timezone.utc), event="Another info message", level="info"),
    ]
    for r in rows:
        session.add(r)
    session.flush()
...
def test_logs_stats(db_session, admin_user):
    ...
    stats = r.json()
    assert stats["total"] == stats["info"] + stats["success"] + stats["warning"] + stats["error"]
