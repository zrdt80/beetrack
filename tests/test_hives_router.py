...
def test_create_hive_success(client: TestClient, db_session, admin_user):
    ...
    apiary = models.Apiary(
        name="Test Apiary", location="Test Location", owner_id=admin_user.id
    )
    db_session.add(apiary)
    db_session.commit()

    hive_data = {"name": "Test Hive", "status": "active", "apiary_id": apiary.id}
...
def test_create_hive_missing_apiary(client: TestClient, db_session, admin_user):
    ...
    hive_data = {"name": "Test Hive", "status": "active"}
...
def test_create_hive_apiary_not_found(client: TestClient, db_session, admin_user):
    ...
    hive_data = {"name": "Test Hive", "status": "active", "apiary_id": 99999}
...
def test_create_hive_duplicate_name(client: TestClient, db_session, admin_user):
    ...
    existing_hive = models.Hive(
        name="Existing Hive", status="active", apiary_id=apiary.id
    )
    db_session.add(existing_hive)
    db_session.commit()

    hive_data = {"name": "Existing Hive", "status": "active", "apiary_id": apiary.id}
...
def test_list_hives_with_search(client: TestClient, db_session, admin_user):
    ...
    apiary = models.Apiary(
        name="Searchable Apiary", location="Test Location", owner_id=admin_user.id
    )
    db_session.add(apiary)
    db_session.flush()
...
def test_update_hive_success(client: TestClient, db_session, admin_user):
    ...
    update_data = {"name": "Updated Hive", "status": "inactive", "apiary_id": apiary.id}
...
def test_update_hive_not_found(client: TestClient, db_session, admin_user):
    ...
    update_data = {"name": "Updated Hive", "status": "inactive"}
...
def test_update_hive_duplicate_name(client: TestClient, db_session, admin_user):
    ...
    update_data = {"name": "Hive One", "status": "active", "apiary_id": apiary.id}
...
def test_delete_hive_success(client: TestClient, db_session, admin_user):
    ...
    deleted_hive = (
        db_session.query(models.Hive).filter(models.Hive.id == hive.id).first()
    )
...
def test_delete_hive_owner_permission(client: TestClient, db_session):
    ...
    apiary = models.Apiary(
        name="Owner Apiary", location="Test Location", owner_id=regular_user.id
    )
    db_session.add(apiary)
    db_session.flush()
    ...
    with client_as(db_session, regular_user) as c:
        response = c.delete(f"/hives/{hive.id}")
        assert response.status_code == 204
