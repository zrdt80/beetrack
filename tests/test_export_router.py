...
def create_order(session, user, items, status="completed", date=None):
    o = models.Order(
        user_id=user.id, status=status, date=date or datetime.now(timezone.utc)
    )
    session.add(o)
    session.flush()
...
def create_hive(
    session, apiary, name="Hive 1", status="active", last_inspection_date=None
):
    h = models.Hive(
        name=name,
        apiary_id=apiary.id,
...
def test_admin_export_orders_csv_success(db_session, admin_user):
    ...
    with client_as(db_session, admin_user) as client:
        ...
        assert (
            "attachment; filename=orders.csv" in cd
            or 'attachment; filename="orders.csv"' in cd
        )
        assert r.content.startswith(
            b"Order ID,User ID,Date,Status,Product ID,Quantity,Price Each,Total"
        )
...
def test_admin_export_inspections_pdf_success(db_session, admin_user):
    ...
    create_inspection(
        db_session, hive, temperature=28.5, disease="none", notes="All good"
    )
    with client_as(db_session, admin_user) as client:
...
def test_export_filtered_orders_as_regular_user_only_own(
    db_session, admin_user, regular_user
):
    ...
    with client_as(db_session, regular_user) as client:
...
def test_export_filtered_inspections_csv_with_permissions(
    db_session, admin_user, regular_user
):
    ...
    with client_as(db_session, regular_user) as client_user:
        ...
        assert (
            r_denied.json()["message"]
            == "No inspections matched filters or access denied"
        )
...
def test_export_filtered_hives_pdf_and_csv(db_session, admin_user, regular_user):
    ...
    create_hive(
        ...
        apiary,
        name="H-A",
        last_inspection_date=datetime.now(timezone.utc) - timedelta(days=10),
    )
    ...
    admin_user.role = models.UserRole.admin
    db_session.flush()
    with client_as(db_session, admin_user) as client_admin:
...
def test_export_filtered_apiaries_csv_with_counts(db_session, admin_user, regular_user):
    ...
    with client_as(db_session, regular_user) as client_user:
        ...
        assert (
            lines[0]
            == "Apiary ID,Name,Location,Owner ID,Created At,Description,Member Count,Hive Count"
        )
        assert len(lines) == 2
