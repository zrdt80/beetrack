...
from tests.conftest import _create_user, _ensure_permission, client_as
from datetime import datetime, timezone


def create_test_apiary(
    db: Session,
    owner: models.User,
    name: str = "Test Apiary",
...
def test_create_apiary_success(client: TestClient, db_session: Session, admin_user):
    ...
    apiary = (
        db_session.query(models.Apiary)
        .filter(models.Apiary.name == payload["name"])
        .first()
    )
...
def test_create_apiary_with_description(
...
def test_create_apiary_insufficient_permissions(
    client: TestClient, db_session: Session, regular_user
):
    payload = {"name": "Test Apiary", "location": "Test Location"}

    with client_as(db_session, regular_user) as c:
        response = c.post("/apiaries/", json=payload)
...
def test_list_apiaries_as_owner_only_sees_own_and_member(
    ...
    with client_as(db_session, owner) as c:
        response = c.get("/apiaries/")
        assert response.status_code == 200
        data = response.json()
        assert len(data["items"]) == 2
        apiary_names = [item["name"] for item in data["items"]]
...
def test_get_apiary_success(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.get(f"/apiaries/{apiary.id}")
        assert response.status_code == 200
        data = response.json()
...
def test_get_apiary_access_denied(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, other_user) as c:
        response = c.get(f"/apiaries/{apiary.id}")
        assert response.status_code == 403
...
def test_update_apiary_success(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.put(f"/apiaries/{apiary.id}", json=payload)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == payload["name"]
        assert data["location"] == payload["location"]
...
def test_update_apiary_admin_access(
    client: TestClient, db_session: Session, admin_user
):
    ...
    payload = {"name": "Admin Updated", "location": "Admin Location"}
...
def test_delete_apiary_success(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.delete(f"/apiaries/{apiary.id}")
        assert response.status_code == 204
        deleted_apiary = (
            db_session.query(models.Apiary)
            .filter(models.Apiary.id == apiary.id)
            .first()
        )
        assert deleted_apiary is None


def test_delete_apiary_admin_access(
    client: TestClient, db_session: Session, admin_user
):
    admin_user.role = models.UserRole.admin
    db_session.commit()
...
def test_list_apiary_members(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.get(f"/apiaries/{apiary.id}/members")
        assert response.status_code == 200
        data = response.json()
        assert len(data["items"]) == 2
...
def test_list_members_with_search(client: TestClient, db_session: Session):
    ...
    for user, role in [
        (member1, models.ApiaryRole.worker),
        (member2, models.ApiaryRole.manager),
    ]:
        membership = models.ApiaryMember(
    ...
    with client_as(db_session, owner) as c:
        response = c.get(f"/apiaries/{apiary.id}/members?q=john")
        assert response.status_code == 200
        data = response.json()
        assert len(data["items"]) == 1
...
def test_update_member_role_success(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.patch(f"/apiaries/{apiary.id}/members/{member.id}", json=payload)
        assert response.status_code == 200
        data = response.json()
        assert data["role"] == "manager"


def test_update_member_role_owner_cannot_change_own(
    client: TestClient, db_session: Session
):
    owner = _create_user(db_session, "owner")
    apiary = create_test_apiary(db_session, owner, "Test Apiary")
    ...
    with client_as(db_session, owner) as c:
        response = c.patch(f"/apiaries/{apiary.id}/members/{owner.id}", json=payload)
...
def test_remove_member_success(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.delete(f"/apiaries/{apiary.id}/members/{member.id}")
        assert response.status_code == 204
        updated_membership = (
            db_session.query(models.ApiaryMember)
            .filter(
...
def test_remove_member_owner_cannot_remove_self(
    client: TestClient, db_session: Session
):
    owner = _create_user(db_session, "owner")
    apiary = create_test_apiary(db_session, owner, "Test Apiary")
    ...
    with client_as(db_session, owner) as c:
        response = c.delete(f"/apiaries/{apiary.id}/members/{owner.id}")
        assert response.status_code == 400
...
def test_invite_member_success(client: TestClient, db_session: Session):
    ...
    payload = {"email": invitee.email, "role": "worker"}

    with client_as(db_session, owner) as c:
        response = c.post(f"/apiaries/{apiary.id}/invitations", json=payload)
        assert response.status_code == 200
        data = response.json()
...
def test_invite_member_nonexistent_user(client: TestClient, db_session: Session):
    ...
    payload = {"email": "nonexistent@example.com", "role": "worker"}
...
def test_accept_invitation_success(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, invitee) as c:
        response = c.post(f"/apiaries/invitations/accept/test_token")
        assert response.status_code == 200
        data = response.json()
...
def test_accept_invitation_wrong_email(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, wrong_user) as c:
        response = c.post(f"/apiaries/invitations/accept/test_token")
        assert response.status_code == 403
...
def test_list_apiary_hives(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.get(f"/apiaries/{apiary.id}/hives")
        assert response.status_code == 200
        data = response.json()
        assert len(data["items"]) == 2
...
def test_create_hive_in_apiary_success(client: TestClient, db_session: Session):
    ...
    payload = {"name": "New Hive", "status": "active"}

    with client_as(db_session, owner) as c:
        response = c.post(f"/apiaries/{apiary.id}/hives", json=payload)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == payload["name"]
...
def test_create_hive_duplicate_name_in_apiary(client: TestClient, db_session: Session):
    ...
    existing_hive = models.Hive(
        name="Duplicate Name", status="active", apiary_id=apiary.id
    )
    db_session.add(existing_hive)
    db_session.commit()

    payload = {"name": "Duplicate Name", "status": "active"}

    with client_as(db_session, owner) as c:
        response = c.post(f"/apiaries/{apiary.id}/hives", json=payload)
        assert response.status_code == 400
        assert (
            "Hive with this name already exists in this apiary"
...
def test_create_hive_manager_permission(client: TestClient, db_session: Session):
    ...
    payload = {"name": "Manager Hive", "status": "active"}

    with client_as(db_session, manager) as c:
        response = c.post(f"/apiaries/{apiary.id}/hives", json=payload)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == payload["name"]
...
def test_create_hive_worker_permission_denied(client: TestClient, db_session: Session):
    ...
    payload = {"name": "Worker Hive", "status": "active"}

    with client_as(db_session, worker) as c:
        response = c.post(f"/apiaries/{apiary.id}/hives", json=payload)
        assert response.status_code == 403
...
def test_transfer_ownership_success(
    ...
    payload = {"new_owner_user_id": new_owner.id}
...
def test_transfer_ownership_non_admin_denied(client: TestClient, db_session: Session):
    ...
    with client_as(db_session, owner) as c:
        response = c.post(f"/apiaries/{apiary.id}/transfer-ownership", json=payload)
        assert response.status_code == 403
        assert "Only admin can transfer ownership" in response.json()["message"]


def test_transfer_ownership_same_owner(
    client: TestClient, db_session: Session, admin_user
):
    admin_user.role = models.UserRole.admin
    db_session.commit()
    ...
    payload = {"new_owner_user_id": owner.id}
...
def test_add_member_direct_admin_success(
    client: TestClient, db_session: Session, admin_user
):
    admin_user.role = models.UserRole.admin
    db_session.commit()
    ...
    payload = {"user_id": new_member.id, "role": "manager"}
...
def test_add_member_direct_non_admin_denied(client: TestClient, db_session: Session):
    ...
    payload = {"user_id": new_member.id, "role": "worker"}

    with client_as(db_session, owner) as c:
        response = c.post(f"/apiaries/{apiary.id}/members", json=payload)
        assert response.status_code == 403
        assert "Only admin can add members directly" in response.json()["message"]


def test_add_member_direct_owner_already_member(
    client: TestClient, db_session: Session, admin_user
):
    admin_user.role = models.UserRole.admin
    db_session.commit()
    ...
    payload = {"user_id": owner.id, "role": "worker"}
